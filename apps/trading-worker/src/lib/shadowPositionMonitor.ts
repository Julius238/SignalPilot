 ...
 * aktiv gesetzt").
 *
 * P5 adds one further trigger `resolveCandleExit` does not know about: a
 * pending, unacknowledged critical `RiskEvent` from the post-fill net-CRV
 * recheck (`shadowFillPersistence.ts`; ADR 0009). If no ordinary stop/TP/time
 * trigger fires on a candle and such a finding exists for the position, this
 * module forces a `MANUAL_RISK_CLOSE` at that candle's close instead of
 * leaving the position open under the ordinary plan.
 */
...
export async function monitorPositionForCandle(
  ...
  if (existingExitOrder !== null) {
    ...
    if (!resolution.triggered || resolution.trigger === null || resolution.referencePrice === null) {
      ...
      // never cleared the post-fill net-CRV minimum, or an admin-requested
      // manual risk close (P6, `lib/manualRiskClose.ts`), still needs a
      // forced, risk-reducing close rather than an ordinary hold
      // (docs/trading/decisions/0009-post-fill-net-crv-recheck.md). The
      // pending, unacknowledged critical finding is the durable record of
      // that decision; the request arose after this candle's own open was
      // already used for the entry fill (or was filed at an arbitrary time
      // between candles), so the earliest honest reference is this candle's
      // close (docs/trading/07, "kommt die Anforderung erst nach Open, close
      // der nächsten vollständig beobachtbaren Candle").
      const forcedExit = await database.riskEvent.findFirst({
        where: {
          shadowPositionId: position.id,
          severity: "CRITICAL",
          acknowledgedAt: null,
      ...
      referencePrice = decimalString(candle.close);
      triggerType = ShadowFillTriggerType.MANUAL_RISK_CLOSE;
    } else {
