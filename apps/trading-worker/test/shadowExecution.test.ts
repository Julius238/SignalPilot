...
function seedWorld(overrides: { readonly takeProfitPrice?: string } = {}) {
  ...
  // Default clears the post-fill net-CRV recheck (~2.07 net at the actual
  ...
  const takeProfitPrice = overrides.takeProfitPrice ?? "25450.000000000000";
...
describe("entry fill — spread, slippage, fee and ledger/portfolio consistency", () => {
  ...
  it("never opens a normal position when the post-fill net-CRV recheck falls short", async () => {
    ...
    const { database, tables, candidate, candleC1 } = seedWorld({ takeProfitPrice: "25350.000000000000" });
    const orderResult = await createShadowOrderForCandidate(database as never, {
      tradeCandidateId: candidate.id,
      asOf: new Date("2026-08-02T09:00:05.000Z"),
      codeVersion: CODE_VERSION,
    ...
    const forcedExitEvents = tables
      .get("riskEvent")!
      .rows.filter((row) => row.shadowPositionId === position.id && row.severity === "CRITICAL");
    assert.equal(forcedExitEvents.length, 1);
    assert.ok(forcedExitEvents[0].acknowledgedAt == null, "the forced-exit finding must start unacknowledged");
    assert.equal(forcedExitEvents[0].reasonCode, "POST_FILL_NET_CRV_BELOW_MINIMUM");

    const markedEvents = tables
      .get("shadowPositionEvent")!
      .rows.filter((row) => row.shadowPositionId === position.id && row.type === "MARKED");
  ...
  it("cancels the unfilled remainder and releases its reserve on a partial fill that falls short", async () => {
    ...
    const { database, tables, portfolio, candidate, candleC1 } = seedWorld({ takeProfitPrice: "25350.000000000000" });
    tables.get("candle")!.update({ where: { id: candleC1.id }, data: { volume: "0.1" } });
    ...
    const reservedAtAcceptance = Number.parseFloat(
      String(tables.get("shadowOrder")!.rows.find((row) => row.id === orderResult.shadowOrderId)!.reservedQuoteAmount)
    );
    ...
    assert.ok(Number.parseFloat(String(order.filledQuantity)) > 0, "the partial fill itself must still stand");
    ...
    const releaseEntries = tables
      ...
      .rows.filter((row) => row.type === "RELEASE" && row.shadowOrderId === order.id);
    assert.equal(releaseEntries.length, 1);
    assert.ok(Number.parseFloat(String(updatedPortfolio.availableCash)) > 10000 - reservedAtAcceptance);
  });
});
...
describe("position monitor — forced MANUAL_RISK_CLOSE after a post-fill CRV shortfall", () => {
  it("closes the position at the next candle's close when no harder stop/TP fires first", async () => {
    ...
    const exitFill = tables.get("shadowFill")!.rows.find((row) => row.triggerType === "MANUAL_RISK_CLOSE");
