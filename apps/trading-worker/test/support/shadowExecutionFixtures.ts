 ...
 */
const RELATION_FILTERS: Record<string, { readonly table: string; readonly foreignKey: string }> = {
  "tradingAlertOutboxAttempt.outbox": { table: "tradingAlertOutbox", foreignKey: "outboxId" }
};

let relationFilterTables: (() => Map<string, Table>) | null = null;

function matchesWhere(row: Row, where: Record<string, unknown> | undefined, tableName?: string): boolean {
  ...
  for (const [key, condition] of Object.entries(where)) {
    ...
    if (key === "OR" && Array.isArray(condition)) {
      ...
      if (!branches.some((branch) => matchesWhere(row, branch, tableName))) return false;
      continue;
    }

    const relation = tableName === undefined ? undefined : RELATION_FILTERS[`${tableName}.${key}`];
    if (relation !== undefined && relationFilterTables !== null) {
      const target = relationFilterTables().get(relation.table);
      const parent = target?.rows.find((candidate) => candidate.id === row[relation.foreignKey]);
      if (parent === undefined) return false;
      if (!matchesWhere(parent, condition as Record<string, unknown>, relation.table)) return false;
      continue;
    }
...
const FILTER_OPERATOR_KEYS = new Set(["in", "notIn", "not", "lte", "gte", "lt", "gt", "is", "none"]);
...
class Table {
  ...
  findUnique(args: { where: Record<string, unknown>; include?: Record<string, unknown>; select?: unknown }): Row | null {
    ...
    const row = this.rows.find((candidate) => matchesWhere(candidate, where, this.name)) ?? null;
  ...
  findFirst(args: {
    ...
    const matches = this.rows.filter((row) => matchesWhere(row, args.where, this.name));
  ...
  count(args: { where?: Record<string, unknown> } = {}): number {
    return this.rows.filter((row) => matchesWhere(row, args.where, this.name)).length;
  }

  deleteMany(args: { where?: Record<string, unknown> } = {}): { count: number } {
    const doomed = this.rows.filter((row) => matchesWhere(row, args.where, this.name));
  ...
  aggregate(args: {
    ...
    const extreme = (fields: Record<string, boolean> | undefined, keepGreater: boolean) => {
      ...
      for (const field of Object.keys(fields ?? {})) {
        ...
        for (const row of matches) {
          const value = row[field];
          if (value === undefined || value === null) continue;
          if (best === null || (keepGreater ? compare(value, best) > 0 : compare(value, best) < 0)) best = value;
        }
        output[field] = best;
      }
      return output;
    ...
    return { _sum: sums, _min: extreme(args._min, false), _max: extreme(args._max, true) };
  }
