...
import { checkShadowStrategyJobAllowed, type TradingFlagSnapshot } from "../lib/tradingSafety.js";
...
export const SHADOW_STRATEGY_SYMBOLS: readonly string[] = CRYPTO_MTF_BREAKOUT_V1_PARAMETERS.allowedSymbols;
...
export async function runShadowGenerateCandidates(
  ...
  for (const symbol of symbols) {
    try {
      const assembled = await assembleStrategyInput(database, { symbol, asOf, codeVersion });
      if (!assembled.ok) {
        assemblerFailures += 1;
        results.push({
          symbol,
        ...
        await writeBotLog(database, "warn", `${JOB_NAME} could not assemble a snapshot`, {
          botRunId: botRun.id,
          correlationId,
          symbol,
          reasonCode: assembled.reasonCode,
      ...
      else if (persisted.outcome === PersistOutcome.IDEMPOTENT_REPLAY) idempotentReplays += 1;
      ...
      errors += 1;
      const message = error instanceof Error ? error.message : "Unknown strategy job error";
      results.push({
        symbol,
        outcome: "ERROR",
      ...
      await writeBotLog(database, "error", `${JOB_NAME} failed for ${symbol}`, {
        botRunId: botRun.id,
        correlationId,
        symbol,
        error: message
  ...
  const status = errors > 0 || conflicts > 0 ? BotRunStatus.FAILED : BotRunStatus.SUCCESS;
  const summary: ShadowGenerateCandidatesSummary = {
    status,
    blocked: false,
