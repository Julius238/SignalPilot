  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <Link className="primary-link secondary-link" href="/dashboard/trading/positions">
            ← Alle Positionen
          </Link>
        }
      />
      ...
      {result.error ? (
        <ErrorState title="Position konnte nicht geladen werden" message={result.error} />
      ) : null}

      {position ? (
        <>
          <div className="grid metrics">
            <MetricCard label="Status" value={<PositionStatusBadge value={position.status} />} />
            <MetricCard
              label="Anfangsmenge"
              value={formatDecimalAmount(position.initialQuantity, { maximumFractionDigits: 6 })}
            />
            <MetricCard
              label="Offene Menge"
              value={formatDecimalAmount(position.openQuantity, { maximumFractionDigits: 6 })}
            />
            <MetricCard
              label="Geschlossene Menge"
              value={formatDecimalAmount(position.closedQuantity, { maximumFractionDigits: 6 })}
            />
            <MetricCard label="Ø Entry" value={formatDecimalAmount(position.averageEntryPrice)} />
            <MetricCard label="Ø Exit" value={formatDecimalAmount(position.averageExitPrice)} />
            <MetricCard label="Stop" value={formatDecimalAmount(position.stopPrice)} />
            <MetricCard label="Take Profit" value={formatDecimalAmount(position.takeProfitPrice)} />
            <MetricCard
            ...
            />
            <MetricCard label="Gebühren" value={formatDecimalAmount(position.feesPaid)} />
          </div>

          <div className="grid two" style={{ marginTop: 16 }}>
            <SectionCard title="P&L-Brücke: Brutto → Fees → Netto">
              <div className="stack-list compact">
                <div className="list-row">
                  <span>Brutto-Entry-Notional</span>
                  <strong>{formatDecimalAmount(position.grossEntryNotional)}</strong>
                </div>
                <div className="list-row">
                  <span>Brutto-Exit-Notional</span>
                  <strong>{formatDecimalAmount(position.grossExitNotional)}</strong>
                </div>
                <div className="list-row">
                  <span>Gebühren</span>
                ...
                <div className="list-row">
                  ...
                  <strong style={{ color: toneColor(decimalTone(position.realizedPnl)) }}>
                    {formatSignedDecimal(position.realizedPnl)}
                  </strong>
                </div>
              </div>
          ...
          <div style={{ marginTop: 16 }}>
            <SectionCard title="Position Events" subtitle="Chronologische Ereignisfolge dieser Position">
                    ...
                    <tbody>
                      {position.events.map((event) => (
                        <tr key={event.id}>
                          ...
                          <td data-label="Menge">{formatDecimalAmount(event.quantity, { maximumFractionDigits: 6 })}</td>
                          <td data-label="Preis">{formatDecimalAmount(event.price)}</td>
                          <td data-label="P&L-Delta">
                            <span style={{ color: toneColor(decimalTone(event.realizedPnlDelta)) }}>
