...
if [ ! -f "packages/database/src/seed.ts" ]; then
  echo "ERROR: Seed file not found at packages/database/src/seed.ts."
  exit 1
fi
...
echo "=== SignalPilot: Production seed via temporary Node container ==="
...
docker run --rm \
  ...
  -w /app \
  "$IMAGE" \
  bash -lc 'set -euo pipefail; corepack enable; corepack prepare "pnpm@${SIGNALPILOT_PNPM_VERSION}" --activate; pnpm install --frozen-lockfile; pnpm db:seed'
