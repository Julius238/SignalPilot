...
function trade(
  overrides: Partial<ClosedShadowTradeInput> & { positionId: string }
): ClosedShadowTradeInput {
  return {
    portfolioId: "pf-1",
...
function input(
  overrides: Partial<ComputeShadowPerformanceInput> = {}
): ComputeShadowPerformanceInput {
  return {
    portfolioId: "pf-1",
    window: WINDOW,
...
function segment(
  ...
  const found = report.segments.find(
    (entry) => entry.segmentType === type && entry.segmentKey === key
  );
...
function overall(report: {
  segments: readonly ShadowPerformanceSegmentResult[];
}) {
  return segment(report, ShadowPerformanceSegment.OVERALL, ALL_SEGMENT_KEY)
    .metrics;
}
...
test("empty data set reports zero counts and null ratios with reasons", () => {
  ...
  assert.equal(
    ...
    1,
    "no segment axis has a bucket without trades"
  );
  ...
  assert.deepEqual(metrics.winRatePct, {
    value: null,
    reason: MetricNullReason.NO_CLOSED_TRADES
  });
  assert.deepEqual(metrics.expectancy, {
    value: null,
    reason: MetricNullReason.NO_CLOSED_TRADES
  });
  assert.deepEqual(metrics.profitFactor, {
    value: null,
    reason: MetricNullReason.NO_CLOSED_TRADES
  });
  assert.deepEqual(metrics.averageWin, {
    value: null,
    reason: MetricNullReason.NO_WINNING_TRADES
  });
  assert.deepEqual(metrics.averageLoss, {
    value: null,
    reason: MetricNullReason.NO_LOSING_TRADES
  });
  assert.deepEqual(metrics.averageR, {
    value: null,
    reason: MetricNullReason.NO_CLOSED_TRADES
  });
  assert.deepEqual(metrics.maxDrawdownAmount, {
    value: null,
    reason: MetricNullReason.NO_CLOSED_TRADES
  });
  assert.deepEqual(metrics.averageHoldMinutes, {
    value: null,
    reason: MetricNullReason.NO_CLOSED_TRADES
  });
});
  ...
  const report = computeShadowPerformance(
    input({
      trades: [
        trade({
          positionId: "p1",
          netPnl: "200.000000000000",
          plannedRiskAmount: "100.000000000000"
        }),
        trade({
          positionId: "p2",
          netPnl: "-100.000000000000",
  ...
  const report = computeShadowPerformance(
    input({
      ...
      trades: [
        ...
        trade({
          positionId: "p2",
          netPnl: "-50.000000000000",
          closedAt: "2026-07-02T01:00:00.000Z"
        }),
        trade({
          positionId: "p3",
          netPnl: "-150.000000000000",
          closedAt: "2026-07-03T01:00:00.000Z"
        }),
        trade({
          positionId: "p4",
          netPnl: "300.000000000000",
          closedAt: "2026-07-04T01:00:00.000Z"
        })
      ]
    })
  );
...
test("drawdown percentage is null without an equity base, the amount stays exact", () => {
  ...
  assert.deepEqual(metrics.maxDrawdownPct, {
    value: null,
    reason: MetricNullReason.NO_EQUITY_BASE
  });
});
...
test("MAE and MFE reverse adverse/favorable prices for synthetic shorts", () => {
      ...
      input({
        trades: [
          trade({
            ...
            direction: "SHORT",
            averageEntryPrice: "100.000000000000",
            // Persisted fields retain their historical price-extreme meaning:
            // maxAdversePrice is the lowest low, maxFavorablePrice the highest high.
            // The metric engine interprets which extreme is adverse by direction.
            maxAdversePrice: "70.000000000000",
            maxFavorablePrice: "110.000000000000"
          })
        ]
      })
    )
...
test("Sharpe and Sortino stay null below the minimum observation count", () => {
  const equityCurve: PortfolioEquityPointInput[] = Array.from(
    { length: 5 },
    (_value, index) => ({
...
test("Sharpe is reported once enough observations exist, Sortino needs a downside day", () => {
  ...
  assert.ok(
    metrics.sharpeRatio.value !== null,
    "Sharpe is computed with 29 observations"
  );
  assert.deepEqual(metrics.sortinoRatio, {
    value: null,
...
test("segments by direction, strategy version, asset, market regime and exit reason", () => {
  const report = computeShadowPerformance(
    input({
      trades: [
        trade({
  ...
  const btc = segment(
    report,
    ShadowPerformanceSegment.ASSET,
    "asset-btc"
  ).metrics;
  assert.equal(btc.closedTrades, 2);
  assert.equal(btc.netPnl, "120.000000000000");
  assert.equal(
    segment(report, ShadowPerformanceSegment.ASSET, "asset-btc").segmentLabel,
    "BTCUSDT"
  );

  assert.equal(
    segment(report, ShadowPerformanceSegment.DIRECTION, "LONG").metrics
      .closedTrades,
    2
  );
  const shorts = segment(
    report,
    ShadowPerformanceSegment.DIRECTION,
    "SHORT"
  ).metrics;
  assert.equal(shorts.closedTrades, 1);
  assert.equal(shorts.netPnl, "-40.000000000000");

  const sv1 = segment(
    report,
    ShadowPerformanceSegment.STRATEGY_VERSION,
    "sv-1"
  );
  assert.equal(sv1.metrics.closedTrades, 2);
  assert.equal(sv1.strategyVersionId, "sv-1");

  const riskOff = segment(
    report,
    ShadowPerformanceSegment.MARKET_REGIME,
    "RISK_OFF"
  ).metrics;
  assert.equal(riskOff.closedTrades, 1);
  assert.equal(riskOff.netPnl, "-40.000000000000");

  const unknownRegime = segment(
    report,
    ShadowPerformanceSegment.MARKET_REGIME,
    UNKNOWN_SEGMENT_KEY
  ).metrics;
  ...
  const stopExits = segment(
    report,
    ShadowPerformanceSegment.EXIT_REASON,
    "STOP"
  ).metrics;
...
test("portfolio-level ratios and the risk funnel are not attributed to a segment", () => {
  ...
  assert.deepEqual(asset.sharpeRatio, {
    value: null,
...
test("the report is deterministic and order independent", () => {
  const trades = [
    trade({
      positionId: "p1",
      netPnl: "10.000000000000",
      closedAt: "2026-07-01T00:00:00.000Z"
    }),
    trade({
      positionId: "p2",
      netPnl: "-4.000000000000",
      closedAt: "2026-07-02T00:00:00.000Z"
    }),
    trade({
      positionId: "p3",
      netPnl: "7.000000000000",
      closedAt: "2026-07-03T00:00:00.000Z"
    })
  ];

  const first = computeShadowPerformance(input({ trades }));
  const second = computeShadowPerformance(
    input({ trades: [...trades].reverse() })
  );
...
test("a different data cut produces a different input hash", () => {
  const base = input({
    trades: [trade({ positionId: "p1", netPnl: "10.000000000000" })]
  });
  const changed = input({
  ...
  assert.notEqual(
    computeShadowPerformance(base).inputHash,
    computeShadowPerformance(changed).inputHash
  );
  assert.notEqual(
    computeShadowPerformance(base).outputHash,
    computeShadowPerformance(changed).outputHash
  );
});
