...
const baseInput = (overrides: Partial<Parameters<typeof computePostFillNetRewardRisk>[0]> = {}) => ({
...
describe("computePostFillNetRewardRisk", () => {
  ...
  it("does not double-charge adverse cost on the already-realised entry price", () => {
    ...
    const perUnitRisk = input.actualAverageEntryPrice.sub(worstStop).add(input.actualEntryFeePerUnit).add(stopFee);
    const expected = input.takeProfitPrice.sub(input.actualAverageEntryPrice).div(perUnitRisk, "FLOOR");
  ...
  it("is not computable when the take profit is not above the actual entry", () => {
    const result = computePostFillNetRewardRisk(baseInput({ takeProfitPrice: d("24100") }));
    assert.equal(result.computable, false);
  });

  it("is not computable when the stop is not below the actual entry", () => {
    const result = computePostFillNetRewardRisk(baseInput({ stopPrice: d("24200") }));
    assert.equal(result.computable, false);
  });

  it("reports a lower net reward/risk when spread and slippage widen", () => {
    const tight = computePostFillNetRewardRisk(baseInput());
    const wide = computePostFillNetRewardRisk(baseInput({ fullSpreadBps: 20, slippageBps: 15 }));
  ...
  it("never rewards a worse fill with a better ratio (a higher actual entry price shrinks the reward)", () => {
    const cheaper = computePostFillNetRewardRisk(baseInput({ actualAverageEntryPrice: d("24100") }));
    const pricier = computePostFillNetRewardRisk(baseInput({ actualAverageEntryPrice: d("24300") }));
