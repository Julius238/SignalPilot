...
Nicht erneut zu erfinden sind: Shadow-only v1; eigene Shadow-Modelle side-by-side; BTCUSDT/ETHUSDT Long sowie ausschließlich synthetische, ungehebelte Shadow-Shorts; keine Exchange-Position und kein Hebel/Margin/Futures/Funding/Borrowing/Liquidationsmodell/LLM; reine versionierte Engines; Portfolio-Ledger; PostgreSQL-Idempotenz/Claims; Stop-first bei unbekannter Intrakerzenfolge; Session standardmäßig STOPPED/Kill aktiv; kein Execution Service/Adapter. Eine spätere echte Short-Ausführung benötigt einen separat freigegebenen Futures-/Margin-fähigen Adapter; ein Spot-Adapter darf Short nie akzeptieren.
