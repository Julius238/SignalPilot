...
- `TradeDirection` ist `LONG | SHORT`. Side, Stop-/TP-Lage, P&L und adverse Rundung werden ausschließlich über die gemeinsamen Direction-Helper abgeleitet; unbekannte Werte blockieren.
- Ein `SHORT` ist ausschließlich eine synthetische, ungehebelte Shadow-Position. Es existieren weder Exchange-Position noch Borrowing, Funding, Margin oder Liquidation.
...
| `key`                    | String, unique         | stabiler Maschinenname, z. B. Legacy `CRYPTO_MTF_BREAKOUT_V1`, `CRYPTO_MTF_BREAKOUT_LONG_V1` oder `CRYPTO_MTF_BREAKDOWN_SHORT_V1` |
...
| `assignmentConfigJson`                        | Json                  | Direction und nur erlaubte assignment-spezifische Caps; aktuelle Versionen pinnen `leverageAllowed=false`, `marginAllowed=false`, `futuresAllowed=false`, Short zusätzlich `syntheticShadowShort=true` |
...
Long und Short sind getrennte Strategien und Assignments. Die Exposure-Scopes bleiben dagegen absichtlich richtungsfrei: `openEntryScopeKey` und `openScopeKey` enthalten nur Portfolio und Asset (plus Orderzweck), niemals `direction`. Damit blockiert eine aktive Entry-Order oder Position automatisch die Gegenrichtung.
...
| `direction`                                                           | `LONG/SHORT`      | muss exakt zu StrategyVersion und Assignment passen |
...
| `direction`                                                        | `LONG/SHORT`                  | unveränderliche Positionsrichtung; muss mit Candidate/Position übereinstimmen |
...
| `direction`                                                           | `LONG/SHORT`         | Long oder synthetischer Shadow-Short                                                |
...
| `reservedCollateral`                                                  | Decimal              | für Long exakt 0; für offenen Short positive, vollständig ungehebelte Quote-Reserve |
...
V1: höchstens eine nichtterminale Position je Portfolio/Asset, unabhängig von der Richtung. Keine positive Mengenmutation nach abgeschlossenem Entry-Order-Prozess. Long und Short werden bei Risiko oder Exposure nie genettet.

Richtungsabhängige Ergebnisformeln:

```text
LONG  unrealizedPnl = quantity × (markPrice - entryPrice)
SHORT unrealizedPnl = quantity × (entryPrice - markPrice)

LONG  realizedPnl = quantity × (exitPrice - entryPrice) - Gebühren
SHORT realizedPnl = quantity × (entryPrice - exitPrice) - Gebühren
```
