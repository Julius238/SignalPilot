...
| `R-005-LONG-ONLY` | Candidate LONG, Entry BUY, ExitPlan SELL; keine Short-/Borrow-Semantik | `SIDE_NOT_ALLOWED` | CRITICAL bei unbekannt, sonst BLOCKER | RuleResult + ggf. RiskEvent | long buy; sell entry; short |
...
| `R-007-STOP-REQUIRED` | Stop vorhanden, >0, unter worstEntry, tickkonform; aktiver ExitPlan kann atomar entstehen | `STOP_MISSING_OR_INVALID` | BLOCKER | RuleResult | valide; null; >=entry; <=0; falscher Tick |
| `R-008-MIN-RR` | `(TP-worstEntry)/(worstEntry-worstStopFill + costs) >= 2.0` und TP > Entry | `REWARD_RISK_BELOW_MINIMUM` | BLOCKER | RuleResult mit actual/limit | genau 2; 1.9999; adverser Fill senkt RR; TP<=entry |
...
| `R-020-REGIME` | Crypto `RISK_ON`, Risk Mode nicht DEFENSIVE/HIGH_RISK/UNKNOWN, Confidence>=60 | `MARKET_REGIME_CONFLICT` | BLOCKER | RuleResult mit Snapshot-ID | risk on 60; 59; risk off; mixed/neutral; stale separat |
...
equity == availableCash + reservedCash + conservative market value
```
